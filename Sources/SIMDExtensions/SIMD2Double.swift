...
public extension SIMD2<Double> {
    ...
    @inlinable func distance(to: SIMD2<Double>) -> Double { simd_distance(self, to) }
    /// Calculates the Euclidean distance between this vector and another vector.
    ///
    /// This function returns the straight-line (L2) distance between `self` and the
    /// given vector `to`, using SIMD-accelerated distance calculation.
    ...
    /// - Returns: The scalar distance as a value of type `Double` representing the
    ///   length of the vector connecting `self` to `to`.
    @inlinable func distanceFast(to: SIMD2<Double>) -> Double { simd_fast_distance(self, to) }
    /// Calculates the Euclidean distance between this vector and another vector.
    ///
    /// This function returns the straight-line (L2) distance between `self` and the
    ...
    /// - Returns: The scalar distance as a value of type `Double` representing the
    ///   length of the vector connecting `self` to `to`.
    @inlinable func distancePrecise(to: SIMD2<Double>) -> Double { simd_precise_distance(self, to) }
    ...
    @inlinable var sqrt: SIMD2<Double> {
        _fallbackSqrt(self)
    }
    ...
    @inlinable func clamp(min: SIMD2<Double>, max: SIMD2<Double>) -> SIMD2<Double> { simd_clamp(self, min, max) }
    ...
    @inlinable func clamp(min: Double, max: Double) -> SIMD2<Double> { simd_clamp(self, Self(repeating: min), Self(repeating: max)) }
    ...
    @inlinable func lerp(with: SIMD2<Double>, by alpha: SIMD2<Double>) -> Self { simd_mix(self, with, alpha) }
    ...
    /// - Note:  
    /// Values of `alpha` outside the range [0, 1] will extrapolate beyond the endpoints.
    @inlinable func lerp(with: SIMD2<Double>, by alpha: Double) -> Self { simd_mix(self, with, Self(repeating: alpha)) }
    ...
    @inlinable func smoothstep(with: SIMD2<Double>, by alpha: SIMD2<Double>) -> Self { simd_smoothstep(self, with, alpha) }
    ...
    @inlinable func pow(_ of: SIMD2<Double>) -> Self {
        if #available(macOS 12.0, iOS 15.0, tvOS 15.0, watchOS 8.0, *) {
    ...
    var asFastArray: [Double] {
        get {
            ...
            withUnsafeBytes(of: v) { self = $0.load(as: SIMD2<Double>.self) }
        }
    }
