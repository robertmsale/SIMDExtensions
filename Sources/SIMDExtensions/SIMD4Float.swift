...
public extension SIMD4<Float> {
    ...
    @inlinable func distance(to: SIMD4<Float>) -> Float { simd_distance(self, to) }
    /// Calculates the Euclidean distance between this vector and another vector.
    ///
    /// This function returns the straight-line (L2) distance between `self` and the
    /// given vector `to`, using SIMD-accelerated distance calculation.
    ...
    /// - Returns: The scalar distance as a value of type `Float` representing the
    ///   length of the vector connecting `self` to `to`.
    @inlinable func distanceFast(to: SIMD4<Float>) -> Float { simd_fast_distance(self, to) }
    /// Calculates the Euclidean distance between this vector and another vector.
    ///
    /// This function returns the straight-line (L2) distance between `self` and the
    ...
    /// - Returns: The scalar distance as a value of type `Float` representing the
    ///   length of the vector connecting `self` to `to`.
    @inlinable func distancePrecise(to: SIMD4<Float>) -> Float { simd_precise_distance(self, to) }
    ...
    @inlinable func clamp(min: SIMD4<Float>, max: SIMD4<Float>) -> SIMD4<Float> { simd_clamp(self, min, max) }
    ...
    @inlinable func clamp(min: Float, max: Float) -> SIMD4<Float> { simd_clamp(self, Self(repeating: min), Self(repeating: max)) }
    ...
    @inlinable func lerp(with: SIMD4<Float>, by alpha: SIMD4<Float>) -> Self { simd_mix(self, with, alpha) }
    ...
    /// - Note:  
    /// Values of `alpha` outside the range [0, 1] will extrapolate beyond the endpoints.
    @inlinable func lerp(with: SIMD4<Float>, by alpha: Float) -> Self { simd_mix(self, with, Self(repeating: alpha)) }
    ...
    @inlinable func smoothstep(with: SIMD4<Float>, by alpha: SIMD4<Float>) -> Self { simd_smoothstep(self, with, alpha) }
    ...
    @inlinable func pow(_ of: SIMD4<Float>) -> Self {
        if #available(macOS 12.0, iOS 15.0, tvOS 15.0, watchOS 8.0, *) {
    ...
    var asFastArray: [Float] {
        get {
            ...
            withUnsafeBytes(of: v) { self = $0.load(as: SIMD4<Float>.self) }
        }
    }
    ...
    ///
    /// - Parameters:
    ///   - euler: A `SIMD3<Float>` containing the rotation angles (in radians) around the x, y, and z axes.
    ...
    ///
    /// - Parameter matrix: A 3x3 rotation matrix of type `simd_float3x3`.
    ///
    ...
    /// - Returns: The rotation angle in radians as a scalar of type `Float`.
    ///
    /// - Note:
