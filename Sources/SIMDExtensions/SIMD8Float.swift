...
public extension SIMD8<Float> {
    ...
    @inlinable func distance(to: SIMD8<Float>) -> Float { simd_distance(self, to) }
    /// Calculates the Euclidean distance between this vector and another vector.
    ///
    /// This function returns the straight-line (L2) distance between `self` and the
    /// given vector `to`, using SIMD-accelerated distance calculation.
    ...
    /// - Returns: The scalar distance as a value of type `Float` representing the
    ///   length of the vector connecting `self` to `to`.
    @inlinable func distanceFast(to: SIMD8<Float>) -> Float { simd_fast_distance(self, to) }
    /// Calculates the Euclidean distance between this vector and another vector.
    ///
    /// This function returns the straight-line (L2) distance between `self` and the
    ...
    /// - Returns: The scalar distance as a value of type `Float` representing the
    ///   length of the vector connecting `self` to `to`.
    @inlinable func distancePrecise(to: SIMD8<Float>) -> Float { simd_precise_distance(self, to) }
    ...
    @inlinable var sqrt: SIMD8<Float> {
        _fallbackSqrt(self)
    }
    ...
    @inlinable func clamp(min: SIMD8<Float>, max: SIMD8<Float>) -> SIMD8<Float> { simd_clamp(self, min, max) }
    ...
    @inlinable func clamp(min: Float, max: Float) -> SIMD8<Float> { simd_clamp(self, Self(repeating: min), Self(repeating: max)) }
    ...
    @inlinable func lerp(with: SIMD8<Float>, by alpha: SIMD8<Float>) -> Self { simd_mix(self, with, alpha) }
    ...
    /// - Note:  
    /// Values of `alpha` outside the range [0, 1] will extrapolate beyond the endpoints.
    @inlinable func lerp(with: SIMD8<Float>, by alpha: Float) -> Self { simd_mix(self, with, Self(repeating: alpha)) }
    ...
    @inlinable func smoothstep(with: SIMD8<Float>, by alpha: SIMD8<Float>) -> Self { simd_smoothstep(self, with, alpha) }
    ...
    @inlinable func pow(_ of: SIMD8<Float>) -> Self {
        if #available(macOS 12.0, iOS 15.0, tvOS 15.0, watchOS 8.0, *) {
    ...
    var asFastArray: [Float] {
        get {
            ...
            withUnsafeBytes(of: v) { self = $0.load(as: SIMD8<Float>.self) }
        }
    }
