...
public extension SIMD2<Float> {
    ...
    @inlinable func distance(to: SIMD2<Float>) -> Float { simd_distance(self, to) }
    /// Calculates the Euclidean distance between this vector and another vector.
    ///
    /// This function returns the straight-line (L2) distance between `self` and the
    /// given vector `to`, using SIMD-accelerated distance calculation.
    ...
    /// - Returns: The scalar distance as a value of type `Float` representing the
    ///   length of the vector connecting `self` to `to`.
    @inlinable func distanceFast(to: SIMD2<Float>) -> Float { simd_fast_distance(self, to) }
    /// Calculates the Euclidean distance between this vector and another vector.
    ///
    /// This function returns the straight-line (L2) distance between `self` and the
    ...
    /// - Returns: The scalar distance as a value of type `Float` representing the
    ///   length of the vector connecting `self` to `to`.
    @inlinable func distancePrecise(to: SIMD2<Float>) -> Float { simd_precise_distance(self, to) }
    ...
    @inlinable var sqrt: SIMD2<Float> {
        _fallbackSqrt(self)
    }
    ...
    @inlinable func clamp(min: SIMD2<Float>, max: SIMD2<Float>) -> SIMD2<Float> { simd_clamp(self, min, max) }
    ...
    @inlinable func clamp(min: Float, max: Float) -> SIMD2<Float> { simd_clamp(self, Self(repeating: min), Self(repeating: max)) }
    ...
    @inlinable func lerp(with: SIMD2<Float>, by alpha: SIMD2<Float>) -> Self { simd_mix(self, with, alpha) }
    ...
    /// - Note:  
    /// Values of `alpha` outside the range [0, 1] will extrapolate beyond the endpoints.
    @inlinable func lerp(with: SIMD2<Float>, by alpha: Float) -> Self { simd_mix(self, with, Self(repeating: alpha)) }
    ...
    @inlinable func smoothstep(with: SIMD2<Float>, by alpha: SIMD2<Float>) -> Self { simd_smoothstep(self, with, alpha) }
    ...
    @inlinable func pow(_ of: SIMD2<Float>) -> Self {
        if #available(macOS 12.0, iOS 15.0, tvOS 15.0, watchOS 8.0, *) {
    ...
    var asFastArray: [Float] {
        get {
            ...
            withUnsafeBytes(of: v) { self = $0.load(as: SIMD2<Float>.self) }
        }
    }
...
public extension View {
    ...
    /// taking a `SIMD2<Float>` vector where `x` represents width and `y` represents height,
    ...
    func frame(_ v: SIMD2<Float>, alignment: Alignment = .center) -> some View {
    ...
    /// modifier, accepting optional `SIMD2<Float>` vectors for minimum, ideal, and maximum sizes.
    ...
    ///
    /// This function wraps SwiftUI’s `.position(x:y:)` modifier, accepting a `SIMD2<Float>` vector
    ...
    func position(_ v: SIMD2<Float>) -> some View {
        return self.position(x: CGFloat(v.x), y: CGFloat(v.y))
    }
    /// Applies an offset to the view using a vector for x and y distances.
    ///
    /// This function wraps SwiftUI’s `.offset(x:y:)` modifier, accepting a `SIMD2<Float>` vector
    ...
    func offset(_ v: SIMD2<Float>) -> some View {
    ...
    ///
    /// This function wraps SwiftUI’s `.scaleEffect(x:y:anchor:)` modifier, accepting a `SIMD2<Float>` vector
    ...
    func scaleEffect(_ v: SIMD2<Float>, anchor: UnitPoint = .center) -> some View {
    ...
    /// `SIMD2<Float>` vector for the offset, which specifies horizontal and vertical displacement.
    ///
    /// - Parameters:
    ...
    func shadow(color: Color = Color(.sRGBLinear, white: 0, opacity: 0.33), radius: CGFloat, offset: SIMD2<Float> = SIMD2<Float>(0.0, 0.0)) -> some View {
